...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = 'bold 28px "Outfit", sans-serif' }) {
    autoBind(this);
...
class Media {
  ...
  createShader() {
    ...
    if (this.image && this.image.startsWith('data:')) {
      // base64: dibujar en canvas 1200x1200 para WebGL
      const img = new Image();
      img.onload = () => {
        const canvas = document.createElement('canvas');
        ...
        texture.image = canvas;
        this.program.uniforms.uImageSizes.value = [1200, 1200];
      };
      img.src = this.image;
    } else {
      // URL externa
...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 28px "Outfit", sans-serif', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  onWheel(e) {
    const normalized = e.deltaMode === 1 ? e.deltaY * 24 : e.deltaMode === 2 ? e.deltaY * window.innerHeight : e.deltaY;
    this.scroll.target += (normalized > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2;
...
export default function CircularGallery({ items, bend = 1, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 28px "Outfit", sans-serif', scrollSpeed = 2, scrollEase = 0.05 }) {
  const containerRef = useRef(null);
  useEffect(() => {
    if (!document.getElementById('circular-gallery-styles')) {
      const tag = document.createElement('style');
      tag.id = 'circular-gallery-styles';
